...
<div>{attribute.latitude}</div>
   
  );
  const Address = node.dataElementAttributes.map((attribute, index) => 
    
  <div>{attribute.city} {attribute.street} {attribute.postalCode}</div>
    ...
    const groups = childNodeCount.map((group:TreeNode) => 
    !group.dataPartitions ? <span className="dark:text-white dark:border-white">{group.name}{" , "}</span>:null );

    const channels = childNodeCount.map((channel:TreeNode) => 
    channel.dataPartitions ? <span className="dark:text-white dark:border-white">{channel.name}{" , "}</span>:null );
    ...
    const attributeRows = node.dataElementAttributes.map((attribute, index) => (
      ...
      <TableCell className="dark:text-white dark:border-white">{attribute.name}</TableCell>
        {/* <TableCell>{attribute.text}</TableCell>  */}
        {/* <TableCell>{attribute.mevisMeanType}</TableCell> */}
